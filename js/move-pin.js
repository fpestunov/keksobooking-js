...
(function() {
  var draggedPin = function(evt) {
    ...
    if (window.mapContainer.map.classList.contains("map--faded")) {
      ...
      window.mapContainer.mainPin.removeEventListener(
        "click",
        window.mapContainer.onPinClick
      );
    }
    ...
    var onMouseMove = function(moveEvt) {
      ...
      if (
        currentCoordY >= window.mapContainer.APPLICATION_HEIGHT.MIN &&
        currentCoordY <= window.mapContainer.APPLICATION_HEIGHT.MAX
      ) {
        window.mapContainer.mainPin.style.top = currentCoordY + "px";
      }
      if (
        currentCoordX >= window.mapContainer.APPLICATION_WIDTH.MIN &&
        currentCoordX <=
          window.mapContainer.APPLICATION_WIDTH.MAX -
            window.mapContainer.PIN.WIDTH
      ) {
        window.mapContainer.mainPin.style.left = currentCoordX + "px";
      }
      window.mapContainer.addressField.value =
        window.mapContainer.mainPin.offsetLeft +
        window.mapContainer.PIN.WIDTH / 2 +
        ", " +
        window.mapContainer.mainPin.offsetTop;
    };
