...
mainPin.addEventListener("mousedown", function(evt) {
  ...
  var onMouseMove = function(moveEvt) {
    ...
    if (
      currentCoordY >= APPLICATION_HEIGHT.MIN &&
      currentCoordY <= APPLICATION_HEIGHT.MAX
    ) {
      mainPin.style.top = currentCoordY + "px";
    }
    if (
      currentCoordX >= APPLICATION_WIDTH.MIN &&
      currentCoordX <= APPLICATION_WIDTH.MAX - PIN.WIDTH
    ) {
      mainPin.style.left = currentCoordX + "px";
    }
    addressField.value =
      mainPin.offsetLeft + PIN.WIDTH / 2 + ", " + mainPin.offsetTop;
  };
