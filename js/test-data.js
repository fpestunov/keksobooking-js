  ...
  var getPins = function(count) {
    ...
    for (var i = 0; i < count; i++) {
      similarAds[i] = {
        ...
        location: {
          x: window.utils.getRandomInt(
            0 + window.map.PIN.WIDTH / 2,
            1200 - window.map.PIN.WIDTH / 2
          ),
          y: window.utils.getRandomInt(
            130 + window.map.PIN.HEIGHT / 2,
            630 - window.map.PIN.HEIGHT / 2
          )
        }
      };
    }
